        ...
        <section class="content">
                ...
                <div class="content__inner--grid">
                    ...
                    <div>
                        ...
                        <div class="detail__wrapper">
                            ...
                            @auth
                            <div class="detail__inner">
                                <h3 class="detail__subtitle--pink">Link for guests <button id="copyLink" class="button__simple"><i class="bi bi-clipboard-fill"></i></button></h3>
                                    <p id='linkWishlist' class="detail__data">{{ URL::route( 'guest', ['slug' => $wishlist->slug])}}</p>
                            </div>
                            <div class="settings__container--bottom">
                                @if ($wishlist->expired)
                                    <a class="button__register--bottom" href="{{route('wishlists.edit-wishlist', ['wishlist_id' => $wishlist->id])}}">Reopen wishlist</a>
                                @else
                                    <a class="button__login" href="{{route('wishlists.edit-wishlist', ['wishlist_id' => $wishlist->id])}}">Close wishlist</a>
                                @endif
                                <a class="button__delete" href="{{route('wishlists.delete-wishlist', ['wishlist_id' => $wishlist->id])}}">Delete wishlist</a>
                            </div>
                            @endauth
                        </div>
                    </div>
            ...
            @guest
                ...
                @if (count($articles) === 0)
                    <div class="favorites__container">
                        <img class="favorites__img" src="{{asset('images/favorites.svg')}}" alt="">
                        <p class="favorites__title">This list is still empty</p>
                    </div>
                @else
                    <div class="content__inner--grid">
                        @foreach ($articles as $article)
